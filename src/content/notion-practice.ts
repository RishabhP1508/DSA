...
export const NOTION_PRACTICE: NotionPracticeRow[] = [
  ...
  { source: "notion-export", mainTopic: "Arrays", title: "Product of Array Except Self", url: "https://leetcode.com/problems/product-of-array-except-self/", coverageIds: ["arrays/prefix-sums"], mappedIds: ["prefix-sums"], status: "mapped", rationale: "Prefix-then-suffix accumulation with the operator switched from + to * — the prefix-sums lesson teaches the left-to-right running accumulation and (in its review) the two-pass prefix/suffix idea; the product adaptation is a direct operator swap noted in docs/references.md." },
  ...
  { source: "notion-export", mainTopic: "Strings", title: "Longest Palindromic Substring", url: "https://leetcode.com/problems/longest-palindromic-substring/", coverageIds: ["strings/palindromes"], mappedIds: ["palindromes"], status: "mapped", rationale: "Expand-around-center: reuse the palindromes lesson's two-pointer 'is this a palindrome' check as the expansion test from each of the 2n−1 centers, tracking the longest. Reviewed palindromes: it teaches the center/two-pointer check; the expand-from-every-center loop is the bridge (recorded in docs/references.md)." },
  ...
  { source: "notion-export", mainTopic: "Stacks and Queues", title: "Largest Rectangle in Histogram", url: "https://leetcode.com/problems/largest-rectangle-in-histogram/", coverageIds: ["stacks/monotonic"], mappedIds: ["monotonic-stack"], status: "mapped", rationale: "Monotonic INCREASING stack of bar indices; on each pop the popped bar's height × the width between the new top and the current index gives a candidate area (with a sentinel flush at the end). Reviewed monotonic-stack: it teaches the pop-on-violation index stack (next-greater); the width-on-pop area computation + sentinel is the bridge (recorded in docs/references.md)." },
  ...
  { source: "notion-export", mainTopic: "Dynamic Programming and Recursion", title: "Combination Sum", url: "https://leetcode.com/problems/combination-sum/", coverageIds: ["dp/combinations"], mappedIds: ["dp-combinations", "backtracking"], status: "mapped", rationale: "Backtracking that REUSES each candidate (stay at index i) while subtracting from a running target, pruning when it goes negative. Reviewed dp-combinations: it teaches choose/skip combination backtracking with a start index; the reuse-of-the-same-index + target-pruning is the bridge (recorded in docs/references.md). Distinct from LC 77 Combinations (no reuse), which is in additionalPractice." },
  ...
  { source: "notion-export", mainTopic: "Bit Manipulation", title: "Sum of Two Integers", url: "https://leetcode.com/problems/sum-of-two-integers/", coverageIds: ["bits/logical-ops"], mappedIds: ["bit-logical-ops", "bit-shifts"], status: "mapped", rationale: "Add without '+': sum-bits = a ^ b, carry = (a & b) << 1, loop until carry is 0 (with Python's arbitrary-int caveat needing a mask). Reviewed bit-logical-ops: it teaches AND/OR/XOR/NOT meanings and that XOR is add-without-carry and AND finds carry positions; combining them into the carry-propagation loop is the bridge (recorded in docs/references.md)." },
